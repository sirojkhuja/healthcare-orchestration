...
use App\Modules\Notifications\Application\Services\NotificationEmailDeliveryService;
use App\Shared\Application\Contracts\KafkaConsumerHandler;
use App\Shared\Application\Data\ConsumedKafkaMessage;
use LogicException;

final class NotificationEmailConsumerHandler implements KafkaConsumerHandler
{
    public function __construct(
        private readonly NotificationEmailDeliveryService $notificationEmailDeliveryService,
    ) {}

    #[\Override]
    public function consumerGroup(): string
    {
        return config()->string('medflow.kafka.group_id', 'medflow-local').'.notifications-email';
    }

    #[\Override]
    public function consumerName(): string
    {
        return 'notifications.email.delivery';
    }
    ...
    public function handle(ConsumedKafkaMessage $message): void
    {
        ...
        $this->notificationEmailDeliveryService->deliver($tenantId, $notificationId);
    }
